...
class IntegralRegressionHead(BaseHead):
    ...
    Args:
        ...
        in_featuremap_size (int | sequence[int]): Size of input feature map 头部网络的输入特征图尺寸
    ...
    def __init__(self,
        ...
        num_deconv = len(deconv_out_channels) if deconv_out_channels else 0  # 判断是否进行上采样
        if num_deconv != 0:
            ...
            self.simplebaseline_head = HeatmapHead(  # 如果进行上采样，使用simplebaseline的头部网络结构
    ...
    def _linear_expectation(self, heatmaps: Tensor,
        ...
        heatmaps = heatmaps.mul(linspace).reshape(B, N, -1)  # heatmaps与linspace维度要保持一致
    ...
    def _flat_softmax(self, featmaps: Tensor) -> Tensor:
        ...
        return heatmaps.reshape(-1, N, H, W)  # 返回batchsize, keypoints, height, weight
    ...
    def forward(self, feats: Tuple[Tensor]) -> Union[Tensor, Tuple[Tensor]]:
        ...
        if self.simplebaseline_head is None:
            ...
            feats = self.simplebaseline_head(feats)  # 有上采样则首先进行上采样

        heatmaps = self._flat_softmax(feats * self.beta)  # softmax操作，因此最终返回做loss的heatmap为归一化heatmap

        pred_x = self._linear_expectation(heatmaps, self.linspace_x)  # 期望计算
        pred_y = self._linear_expectation(heatmaps, self.linspace_y)

        if self.debias:  # 是否进行debias
            B, N, H, W = feats.shape
    ...
    def loss(self,  # IPR实际上也是用到了热图监督的
             inputs: Tuple[Tensor],
             batch_data_samples: OptSampleList,
